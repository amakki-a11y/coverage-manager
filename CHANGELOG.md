...
## [Unreleased]
### Fixed
- **PnLPanel `findBBookSymbol` now prefers exact bare match over prefix match** — the previous case-insensitive fix used `bbookSymbols.find(s => s.startsWith(canonical))`, which silently matched the WRONG row whenever a canonical name was a strict prefix of another B-Book symbol present in the same response. e.g. canonical `USOIL` matched `USOILK6-` first (because USOILK6- happened to come before USOIL- in the API response), so USOIL coverage P&L (-23,868) was routed to the USOILK6- row's `coverageData` slot, leaving USOIL- itself rendering `—`. Same bug for `UT100` vs `UT100M6-` and any other "base + futures variant" pair. Fix: try exact match (after stripping trailing `-`/`.`) first; fall back to prefix only if no bare match exists. Verified in browser preview: all 36 P&L rows now populate correctly with their own Cov P&L. Backend + Compare-tab `ExpandedTable` already key by canonical name, so they were unaffected.
