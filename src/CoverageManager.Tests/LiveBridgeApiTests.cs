...
{
    // v2 Phase 2: the Live Bridge feed is the ONLY provider. Blank selects it, and the
    // retired Manager provider is rejected with a pointed migration message so an old
    // config fails fast at startup instead of looking like a feed outage.

    [TestMethod]
    public void Normalize_BlankSelectsLiveBridge()
    {
        Assert.AreEqual(MT5ApiProviders.LiveBridge, MT5ApiProviders.Normalize(null));
        Assert.AreEqual(MT5ApiProviders.LiveBridge, MT5ApiProviders.Normalize(""));
        Assert.AreEqual(MT5ApiProviders.LiveBridge, MT5ApiProviders.Normalize("   "));
    }
    ...
    [TestMethod]
    public void Normalize_RejectsRetiredManagerProvider_WithMigrationHint()
    {
        var ex = Assert.ThrowsException<ArgumentException>(() => MT5ApiProviders.Normalize("Manager"));
    ...
    [TestMethod]
    public void Factory_DefaultsToLiveBridge_AndNeedsNoManagerAccount()
    {
        var factory = new MT5ApiFactory();
        Assert.AreEqual(MT5ApiProviders.LiveBridge, factory.ProviderName);
        Assert.IsFalse(factory.RequiresManagerAccount,
            "v2 has no Manager provider; the bring-up must never read account_settings");
    }
