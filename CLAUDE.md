...
- **Known limit.** After a restart `RequestDeals` covers only the replayed gap plus live deals; today's earlier deals re-enter `DealStore` from Supabase through `DataSyncService`'s startup load. Supabase stays the durable deal record.
