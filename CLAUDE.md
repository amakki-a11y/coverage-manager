...
- `WS /ws/exposure` — Real-time push for the dashboard. Two message types coexist on the same socket:
  ...
  - `{"type":"price_update","data":{prices, timestamp}}` — lightweight price-only frame. Throttled to 20/sec. Fires on every MT5 tick (idempotent — bursts coalesce into the next 50 ms slot). Lets bid prices update at full tick cadence without paying the per-position exposure recompute cost.
