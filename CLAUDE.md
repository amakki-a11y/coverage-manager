...
- **Mapping** ([`FeedMapping`](src/CoverageManager.Connector/LiveBridge/FeedMapping.cs)): `VolumeExt` (lots × 1e8) → `RawPosition.Volume` lots / `RawDeal.VolumeRaw` (1e-4 lot, rounded); `TimeCreate` / `Time` (source seconds) → `TimeMsc` × 1000, the same source-clock numbers the Manager API path uses; prices/money read as `decimal` from the JSON text. **Not on the wire:** deal `Fee` (always 0), account registration / last-access times (0), comments (""). An account frame with `Computed = false` gets `Equity = Balance + Credit`, `Margin = 0`.
