...
│   │   ├── Models/                     # Position, SymbolMapping, ExposureSummary, SymbolPnL, ClosedDeal, TradingAccount, DealRecord, TradeAuditEntry, PriceQuote, SymbolExposure, TradeRecord, DealHistoryWindow
│   │   └── Engines/                    # PositionManager, ExposureEngine, PriceCache, DealStore, DealReconciler
...
│   │   ├── LiveBridgeApi.cs            # IMT5Api over the Live Bridge consumer feed (WSS, resume sequences; see "MT5 API provider")
│   │   ├── LiveBridgeOptions.cs        # `LiveBridge` config section (Url, ApiKey via env, StatePath, timings)
│   │   ├── IMT5ApiDiagnostics.cs       # Optional per-provider counters → /api/exposure/diagnostics.liveBridge
│   │   ├── IMT5DealHistory.cs          # Optional: how far back RequestDeals reaches (DealHistoryWindow), the feed's ghost-delete gate
│   │   ├── LiveBridge/                 # FeedWire (frames), FeedMapping (payload→Raw*), FeedBook (state), FeedSequenceStore, FeedSocket, GroupMask
...
## MT5 API provider (v2: Live Bridge ONLY)
> **v2 Phase 2 (branch `v2-local-postgres`) removed the Manager provider.** `MT5ApiReal`,
> the managed MetaQuotes wrapper references and the native DLLs under `Connector\Libs\`
> are deleted, and `MT5_API_AVAILABLE` is no longer defined. `MT5:Provider` accepts only
> `LiveBridge` (blank also selects it); `Manager` now throws at startup with a migration
> message instead of failing silently. The rows below describing the Manager API apply to
> **v1 / the `live` branch only**.

`MT5ManagerConnection` / `MT5CoverageConnection` never construct the API directly. They receive an [`IMT5ApiFactory`](src/CoverageManager.Connector/IMT5ApiFactory.cs) and call `Create()` on every (re)connect, so the B-Book feed can be swapped without touching the bring-up order, event wiring or reconnect backoff. The provider is resolved once at startup in `Program.cs` from `MT5:Provider`:

| `MT5:Provider` | Implementation | Behaviour |
|---|---|---|
| `Manager` | `MT5ApiReal` | **v1 only — REMOVED in v2.** MetaQuotes Manager API via the native DLLs in `Connector\Libs\`. |
| `LiveBridge` (v2: the only value) | [`LiveBridgeApi`](src/CoverageManager.Connector/LiveBridgeApi.cs) | **The Live Bridge consumer feed** (TheBridge `docs/product-spec.md` §15, proposal 14). One WebSocket to `wss://feed.connecttrader.app:5571/feed/BBcorp-Live` with `Authorization: Bearer <key>` (v2: key read from the file `LiveBridge:ApiKeyFile`, default `C:\ProgramData\CoverageManagerV2\secrets\livebridge_v2_key.txt`, ACL SYSTEM + Administrators; env `LiveBridge__ApiKey` is the alternative and setting both is refused; nothing dials unless `LiveBridge:Enabled=true`); `subscribe` with the four resume sequences (null on first start); `hello` → `snapshot`/`replay` (+ `replay_gap`) → `snapshot_end`/`replay_end` → live `record`/`records`, `heartbeat`, `bye`. Settings in the `LiveBridge` section ([`LiveBridgeOptions`](src/CoverageManager.Connector/LiveBridgeOptions.cs)). |

**What the adapter does (contract rules → code):**
- **Sequences.** Last applied sequence per stream kept durably in `LiveBridge:StatePath` (default `%LOCALAPPDATA%\CoverageManager\livebridge-state.json`, survives a `publish\api` swap; debounced atomic writes, flushed at every handshake end, drop, and dispose) and sent on every connect. A live/replay record with a sequence at or below the last applied of its stream is ignored (counted `skippedBySequence`). Snapshot records are applied regardless of their sequence (full state).
- **Idempotent by identity.** [`FeedBook`](src/CoverageManager.Connector/LiveBridge/FeedBook.cs): positions by position number, accounts by login, last tick per symbol, deals by deal number. Same record twice = no change, no event. A position `update` and an account frame replace the whole record; `delete` closes. A positions **snapshot reconciles** the book: positions it does not mention are closed and raised as `OnPositionDelete`.
- **Events.** `OnTick / OnDealAdd / OnPositionAdd|Update|Delete / OnUserUpdate` are raised for every applied record, snapshot, replay and live alike, gated by `Subscribe*` (so the bring-up order in `MT5ManagerConnection` still holds: nothing fires before it subscribes). A reconnect's replay therefore reaches `DealStore` / `PositionManager` like live data.
- **Queries.** `GetUserLogins(mask)` (MT5-style mask: `*`, `?`, comma list, `!` exclude), `GetPositions`, `GetUserAccount`, `GetTickLast` answer from the book; `RequestDeals` from the deals received in this process (`DealRetentionHours`, default 48). `SelectedAddAll` is a no-op (the symbol filter is the bridge console's).
- **Resilience.** After `SilenceLimitMs` (15 s) without a frame, after a `bye`, or on any socket error the session is dropped and re-dialed with resume, backoff `ReconnectMs` (1 s) doubling to `ReconnectMaxMs` (30 s); the adapter stays **attached** meanwhile (`IsConnected` true, `IsLive` false, book and subscriptions kept) so `MT5ManagerConnection` does not tear it down; after `ReconnectGiveUpAttempts` (20, about nine minutes) failed reconnects in a row it reports disconnected and the connection service starts a fresh session. A fresh session always asks for **snapshots** of positions, accounts and ticks and resumes only deals, whatever the durable sequences say: on 2026-09-11 a fresh process that resumed all four streams replayed only the changes since its sequences onto an empty book (CLIENTS incomplete), and a tear-down after a bridge drop then resumed from the advanced sequences into an empty book (CLIENTS empty). A **first** connection that fails (401 key refused, 403 address, 404 source, no route, handshake timeout) makes `Connect()` return false with the reason in `LastError`, and the caller's own backoff applies. `replay_gap` for ticks is accepted (the last prices follow); for deals it is logged as a warning (those deals are not recoverable from the feed). An empty key fails `Connect()` before dialing. Acks with the applied sequences every `AckEveryMs` (5 s).
- **Mapping** ([`FeedMapping`](src/CoverageManager.Connector/LiveBridge/FeedMapping.cs)): `VolumeExt` (lots × 1e8) → `RawPosition.Volume` lots / `RawDeal.VolumeRaw` (1e-4 lot, rounded); `TimeCreate` / `Time` (source seconds) → `TimeMsc` × 1000, the same source-clock numbers the Manager API path uses; prices/money read as `decimal` from the JSON text; a deal's four money fields `Profit`, `Storage` (swap), `Commission`, `Fee` map one to one (`Fee` joined the contract on 2026-09-11; a payload without it reads as 0). **Not on the wire:** account registration / last-access times (0), comments (""). An account frame with `Computed = false` gets `Equity = Balance + Credit`, `Margin = 0`.
- **Diagnostics.** `/api/exposure/diagnostics.liveBridge` (via [`IMT5ApiDiagnostics`](src/CoverageManager.Connector/IMT5ApiDiagnostics.cs)): state, mode, source, `sourceConnected` (the bridge's own MT5 link, from heartbeats), handshakes/reconnects/byes, frames, applied and skipped per stream, gaps, sequences, state file, book counts. Startup log: `MT5 API provider: …`, `Live Bridge: hello …`, `Live Bridge: snapshot|resume complete …`.
- **Bring-up without `account_settings`.** `IMT5ApiFactory.RequiresManagerAccount` is false for the feed, so `MT5ManagerConnection` skips the Supabase read of the manager row entirely and connects from config (`LiveBridge:Url` + the key from `LiveBridge:ApiKeyFile` or env `LiveBridge__ApiKey`) with a synthetic account (label `LiveBridge feed`, server = feed URL, login 0, group mask `*`). A Supabase outage therefore cannot block the feed bring-up (it did on 2026-09-11, before this). The Manager API still needs the row.
- **Deal history window (the switch-over gate).** `RequestDeals` covers only the deals received in this process (the replayed gap plus live, 48 h retention); the server's older deals are not answerable from the feed. The adapter therefore reports how far back its record reaches through [`IMT5DealHistory`](src/CoverageManager.Connector/IMT5DealHistory.cs) (a `DealHistoryWindow`: never earlier than the earliest deal it holds, moved forward by a deals `replay_gap`, plus `LiveBridge:DealHistoryMarginMs` = 5 min for clock skew between the MT5 server and the bridge; `None` while it holds nothing), surfaced as `MT5ManagerConnection.DealHistory` and on `diagnostics.liveBridge.dealHistory`. Every consumer that compares provider deals with Supabase goes through [`DealReconciler`](src/CoverageManager.Core/Engines/DealReconciler.cs): the nightly reconciliation sweep and `/api/exposure/verify` (+ `/verify/detail`) clamp their window to it and never delete, or report as extra, a Supabase deal older than it (`reconciliation_runs.window_from` shows the clamped start, `notes` the reason; while the feed holds no deals nothing is compared or deleted); the bring-up backfill and `ReloadDeals` keep the in-memory deals the feed cannot re-supply instead of clearing `DealStore`. The Manager API reports `Full` and behaves exactly as before. Today's earlier deals still re-enter `DealStore` from Supabase through `DataSyncService`'s startup load; Supabase stays the durable deal record.
- Unknown provider names throw in `MT5ApiProviders.Normalize` at startup, so a typo fails fast instead of looping in the reconnect backoff. Matching is case-insensitive; blank = `Manager`. Per-environment override: `MT5__Provider=LiveBridge` (env var), no rebuild. `/api/exposure/status.mt5Provider` shows the active one.
- **Tests:** [`LiveBridgeApiTests.cs`](src/CoverageManager.Tests/LiveBridgeApiTests.cs) drives the adapter against a loopback feed that speaks the contract ([`Tests/LiveBridge/FakeFeedServer.cs`](src/CoverageManager.Tests/LiveBridge/FakeFeedServer.cs): upgrade with the key, 401/404, hello, snapshot, replay, gaps, end frames, records, heartbeat, bye) — first start, live records and events, the sequence rule, resume after a restart, reconnect after bye and after silence, snapshot reconciliation, acks, refusals; plus `FeedWireTests`, `FeedMappingTests`, `FeedBookTests`, `FeedSequenceStoreTests`; `DealReconcilerTests` (a Supabase deal older than the earliest feed deal is kept, an in-window ghost is deleted, nothing is deleted while the feed holds nothing, Manager = full history unchanged) and `MT5ManagerConnectionDealHistoryTests` (a reload under the feed keeps the deals it cannot re-supply).

## Data Sync Architecture
- **DataSyncService** (background): every 30 s writes only the deals `DealStore` queued as added or changed since their last successful write, in paced chunks (`DataSync` section: 500 rows per request, at most 20 requests per tick, 250 ms apart, 30 s per request; one probe request per tick while Supabase refuses). Persisted deals older than `RetentionHours` (48) leave memory; `HardCap` (2,000,000) bounds it. Counters on `/api/exposure/diagnostics.dealSync` (`pending` should stay near 0, `consecutiveFailures` 0). See Operational Learnings, "Feed-scale persistence".
...
- **Account sync:** MT5ManagerConnection reads all accounts on connect + every **15 minutes** but writes only the ones whose fields changed since they were last written (`AccountSyncTracker`, money to the cent; the first cycle writes all), 500 per request, 200 ms apart. Counters on `diagnostics.accountSync`.
...
- **Change detection:** a deal that arrives with different values than the copy in memory is recorded by `DealStore` and written to `trade_audit_log` from that copy (`DealChangeAudit`); nothing is read back from Supabase. The nightly sweep compares modifications against the stored deals it already fetched.
- **Batch upserts:** 500 records per request for accounts and deals, with `on_conflict` for dedup, paced as above; under the Live Bridge feed `CashMovementSyncService` idles (the feed pushes admin deals itself, and walking 26,845 logins took hours per cycle).
...
### Provider window (Live Bridge)
Ghost deletion assumes the provider returns the server's full history for the window. Under `MT5:Provider = LiveBridge` it does not (only the deals received since the feed's resume point, 48 h retention), so the sweep runs through `DealReconciler.Plan(...)` with `MT5ManagerConnection.DealHistory`: the window is clamped to the earliest deal the feed holds, Supabase deals older than that are kept unchecked (never ghosts), and while the feed holds no deals nothing is compared or deleted. `window_from` on the run row is the clamped start and `notes` says why. `/api/exposure/verify?delete=true` and `/verify/detail` follow the same rule; the Manager API (`DealHistoryWindow.Full`) is unaffected.
...
- `WS /ws/exposure` — Real-time push for the dashboard. Three message types coexist on the same socket:
  ...
  - `{"type":"deal_settled","data":{canonicalKey, source, dealId, settledDelta, dealTimeUtc}}` — per-deal SETTLED-delta frame (Phase 2.19). Fires the moment a closed B-Book deal arrives via `OnDealReceived`, no throttling (deals are sparse vs ticks). `settledDelta = (entry IN (1,2,3) ? profit + swap : 0) + commission + fee` — mirrors the `aggregate_bbook_settled_pnl` SQL formula. `canonicalKey` is normalized identically to the SQL function so the frontend overlay lands on the same row the next REST refresh will see. Net P&L tab adds the delta to its cached settled value, then drops the accumulator on the next 30 s REST refresh so deltas don't double-count once Supabase catches up. Coverage settled stays REST-only — collector has no event hook into the C# backend.
...
> **v1 only.** v2 (branch `v2-local-postgres`, Phase 2) deleted the Manager connector and
> every native DLL, so nothing P/Invokes any more and this hazard is retired there.
...
### Supabase TLS reset during cold start can leave mappings cache empty (Phase 2.21)

Observed once on 2026-05-07 17:57 UTC: the Supabase HTTPS connection failed with `SocketException 10054 — connection forcibly closed by remote host` during the first seconds of `coverage-api` startup. Old code returned an empty list on failure and continued, leaving every symbol with the amber UNMAPPED badge until something downstream (a dealer mapping CRUD action via the Mappings tab) triggered a `LoadMappings` reload. There was no retry, no periodic refresh, no health metric.

**Fix shipped (Phase 2.21):**
- `SupabaseService.GetMappingsAsync` now retries 3x with 500/1500/4500 ms exponential backoff (generic `RetryAsync<T>` helper colocated in the same file for future reuse on other Supabase calls).
- New `MappingRefreshService : BackgroundService` ticks every 60 s, re-fetches active mappings, and atomically replaces the in-memory cache. Auto-heals startup failures within one minute.
- **Empty-result safety:** the refresh path will NOT call `PositionManager.LoadMappings([])` on an empty result — `LoadMappings` does `Clear()` first, so passing an empty list would surface UNMAPPED badges across the entire dashboard. Instead it bumps `consecutiveFailures` and leaves the cache alone.
- `Program.cs` startup mapping load routed through `MappingRefreshService.RefreshOnceAsync` so the diagnostics counters are populated immediately on a successful boot.
- Cache state visible at `/api/exposure/diagnostics.mappings = {lastFetchCount, lastFetchAtUtc, lastFetchOk, consecutiveFailures}`.

**If a dealer reports missing mappings after a restart, check `/api/exposure/diagnostics.mappings.lastFetchCount` first** — should be > 0 within 60 s of startup. If it stays at 0 with `consecutiveFailures > 1`, Supabase is unreachable from the VPS (provider-firewall block, expired DNS, supabase.com outage) and the dashboard will show UNMAPPED badges for every symbol until the next successful refresh.

### Feed-scale persistence: never re-write the whole store (2026-09-11)

The first cutover of the CLIENTS side to the Live Bridge feed ran eight hours with the feed itself flawless (0 reconnects, 0 gaps) and was rolled back because Supabase stopped answering. Cause: `DataSyncService` re-upserted the entire in-memory `DealStore` every 30 s and read every one of those deals back for change detection. At 40 Manager logins that was a few thousand rows; the feed's 26,845 accounts produce about 20,000 deals an hour, so by 14:00Z each tick sent 70,000 rows (about 16,000 reads and 3,800 batch writes per hour on `/rest/v1/deals`), Supabase's statement timeouts became request timeouts, and its API gateway answered 503/522 for hours. The `trading_accounts` sync wrote all 26,845 rows every 15 min on top.

**Fix shipped:** (a) `DealStore` queues only deals added or changed since their last successful write (versioned, so a change that lands during a write stays pending), records value changes for the audit trail from the in-memory copy, and is bounded (`DataSync:RetentionHours`, `DataSync:HardCap`). (b) `DataSyncService` writes the pending set in paced chunks with a per-request timeout and one probe request per tick while Supabase refuses; `diagnostics.dealSync`. (c) `MT5ManagerConnection` writes only changed accounts, paced; `diagnostics.accountSync`. (d) `CashMovementSyncService` idles under the feed. (e) `FeedBook` indexes deals per login so `RequestDeals` for 26,845 logins does not scan every deal held. (f) The full-state broadcast takes today's closed P&L from a cache that recomputes only when a deal changes and at most once a second. (g) The nightly sweep compares modifications against the stored deals it already fetched. (h) The bring-up backfill window ends a day ahead of UTC because deal stamps are the server's clock (UTC+3 here); with a window ending at UTC "now" the feed's newest deals never reached `DealStore` (seen at the 22:35Z re-cutover: 3,970 replayed, 1,180 backfilled until a reload with a wider window). Watch `diagnostics.dealSync.pending` and `consecutiveFailures` after any cutover.

**Symbol mapping gap seen at the cutover:** the feed's client symbols carry group-specific suffixes (`UT100-20`, `US30-10`, `DE40U6-`, `ES500.U6`) with no `symbol_mappings` row, while FXGROW posts `UT100.c`. An unmapped symbol keeps its raw name as canonical on both sides, so the client `UT100-20` row and the coverage `UT100.c` position never meet and the hedge reads 0%. Fix in Settings → Mappings: one row per client symbol variant (canonical `UT100`, B-Book `UT100-20`, coverage `UT100.c`, each side's contract size), likewise for `US30-10`; the UNMAPPED badge marks the rows that need one.
...
- [x] Phase 2.18: WebSocket `permessage-deflate` compression — both `/ws/exposure` and `/ws/bridge` accept calls now pass `WebSocketAcceptContext { DangerousEnableCompression = true }` so frames travel ~1/4 the size on the wire (~78% savings measured: 5-frame sample 110,624 → ~24,769 bytes). RFC 7692 standard, browsers negotiate the extension automatically — no client change. The "Dangerous" prefix in ASP.NET Core flags CRIME-style attacks when mixing compressed+secret data; doesn't apply here (WS payload is public market data, no auth tokens). Critical for dealers on WAN where the live VPS uplink is the bottleneck — Phase 2.16 + 2.17 made the backend ship 20 Hz cleanly, this makes that 20 Hz reach the browser through a slow pipe.
...
- [x] Phase 2.19: Live SETTLED column — Phases 2.16/2.17 made floating P&L tick live, but the Net P&L tab's `Settled` column was still REST-only with a 30s polling cadence + 30s `DataSyncService` Supabase write cycle (worst-case 60s lag from a deal close to the cell ticking). Now closes the loop: (a) `ExposureBroadcastService.BroadcastDealSettled(canonicalKey, settledDelta, dealTime, dealId, source)` ships an unthrottled `deal_settled` WS frame the moment a deal arrives. (b) `MT5ManagerConnection.OnDealReceived` invokes the new `onDealSettled` callback (skipping action ≥ 2 admin balance/credit deals). (c) `Program.cs` computes `delta = (entry IN (1,2,3) ? profit + swap : 0) + commission + fee` — same formula as the `aggregate_bbook_settled_pnl` SQL function — and uses a `NormalizeCanonicalKey` helper that mirrors the SQL's UPPER + strip-trailing-`.xxx`/`-` rules so the live overlay lands on the same row the next REST refresh will see. (d) Frontend `useExposureSocket` adds a `pendingSettled: Map<canonicalKey, {bbook, coverage, dealIds}>` accumulator with dedupe-by-`dealId` (guards against reconnect dupes) plus a `clearSettledDeltas()` action. (e) `PeriodPnLPanel` overlays `pendingSettled` onto each row's `settled` / `net` + the TOTAL row, calls `clearSettledDeltas()` after every REST refresh so deltas don't double-count once Supabase catches up. (f) Verified end-to-end: a synthetic `+250` delta for XAUUSD bumps both per-row and TOTAL Settled by exactly $250 with ~50ms latency. Coverage settled stays REST-only (collector has no event hook into the C# backend).
